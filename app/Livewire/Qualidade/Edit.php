...
class Edit extends Component
{
    ...
    // Unificado: Array dinâmico agora se chama checklists
    ...
    public function mount($id) 
    {
        ...
        // Carrega os checklists do banco para o array
    ...
    public function update()
    {
        $this->validate([
            ...
            'selectedUsers' => 'array',
            'checklists.*.description' => 'required|string|max:255', // Validação unificada
        ], [
        ...
        // Loop no array unificado
        foreach ($this->checklists as $stepData) {
            if (isset($stepData['id']) && $stepData['id']) {
                $check = QualityChecklist::find($stepData['id']);
                if ($check) {
                    $check->update(['description' => $stepData['description']]);
