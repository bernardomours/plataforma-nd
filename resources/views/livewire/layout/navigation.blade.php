...
<div x-data="{ open: false }">
    <aside class="hidden md:flex flex-col fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 z-50">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-2">
            ...
            <div x-data="{ isOpen: true }" class="pt-4 pb-1">
             ...
             <!-- fim da seção ocupação -->

             <!-- inicio seção dministração -->
              @role('admin')
              <div x-data="{ isOpen: true }" class="pt-4 pb-1">
                <button @click="isOpen = !isOpen" class="w-full flex items-center justify-between px-3 pb-2 text-xs font-semibold text-gray-500 hover:text-gray-700 transition-colors focus:outline-none">
    ...
    <aside :class="{'translate-x-0': open, '-translate-x-full': ! open}" class="fixed inset-y-0 left-0 w-64 bg-gray-900 z-50 transform transition-transform duration-300 md:hidden flex flex-col">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-2">
            <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('dashboard') ? 'bg-gray-800 text-gray-100' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-100' }} transition-colors">
                <span class="font-medium text-sm">Painel de Controle</span>
            </a>
            </nav>
    </aside>
</div>